...
def generate_request():
    ...
    if req >= 1:
        ...
        print(f"В черзі залишилося {queue.qsize()} необроблених заявок: {'+' * queue.qsize()}.\n")
...
def process_request():
    ...
    if DIFFICULT_CASE > 0:
        ...
        print(f"В черзі залишилось {queue.qsize()} необроблених заявок: {'+' * queue.qsize()}\n")
        DIFFICULT_CASE = 0
        return
...
while True:
    ...
    if input("Press 'q' for quit, any key + Enter for continue ->") in "q":
        print("\n")
        break
